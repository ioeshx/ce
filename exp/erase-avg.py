...
def edit_model(args, pipeline, target_concepts, anchor_concepts, retain_texts, baseline=None, chunk_size=128, emb_size=768, device="cuda"):
    ...
    ## 这里将其保留为列表，用于分别计算每个概念的 delta
    sum_target_target_list, sum_anchor_target_list = sum_target_target, sum_anchor_target
    
    ## 计算平均值用于 erase_weight 的初始化（针对 aug_num 的情况使用）
    sum_target_target_mean, sum_anchor_target_mean = torch.stack(sum_target_target).mean(0), torch.stack(sum_anchor_target).mean(0)
    ...
    for (layer_name, layer_weight) in tqdm(edit_dict.items(), desc="Model Editing"):
        ...
        erase_weight_mean = layer_weight @ (sum_anchor_target_mean - sum_target_target_mean) @ (I + sum_target_target_mean).inverse()        
        ...
        for j in range(0, len(layer_ret_embs), chunk_size):
            ...
            #         [chunk_ret_embs, generate_perturbed_embs(chunk_ret_embs, P0_min, erase_weight_mean, num_per_sample=args.aug_num)], dim=0
            #     )
        ...
        # 逐个概念应用闭式解公式进行 Delta 计算
        delta_weights = []
        for stt, sat in zip(sum_target_target_list, sum_anchor_target_list):
            delta = layer_weight @ (sat - stt) @ P @ (stt @ P + I).inverse()
            delta_weights.append(delta)
            
        # 根据聚合模式汇聚 Delta
        if args.agg_mode == 'sum_delta':
            print("Using sum of deltas across concepts for model editing.")
            final_delta_weight = torch.stack(delta_weights).sum(0)
        elif args.agg_mode == 'avg_delta':
            print("Using average delta across concepts for model editing.")
            final_delta_weight = torch.stack(delta_weights).mean(0)
        else:
            raise ValueError(f"Unknown agg_mode: {args.agg_mode}")
            
        edit_dict[layer_name] = layer_weight + final_delta_weight

    print(f"Current model status: Edited {str(target_concepts)} into {str(anchor_concepts)}, with agg mode {args.agg_mode}")
    return edit_dict
...
if __name__ == '__main__':
    ...
    # Hyperparameters
    parser.add_argument('--agg_mode', type=str, default='avg_delta', choices=['avg_delta', 'sum_delta'], 
                        help='How to aggregate deltas across different concepts.')

...
from diffusers import AutoPipelineForText2Image, StableDiffusionPipeline, DPMSolverMultistepScheduler
import random

from util.utils import str2bool
from util.template import imagenet_templates, imagenet_templates_extend, imagenet_classes, CIFAR100_classes, painting_templates
...
def is_sdxl_pipeline(pipeline):
    return (
        hasattr(pipeline, "text_encoder_2")
        and getattr(pipeline, "text_encoder_2") is not None
        and hasattr(pipeline, "tokenizer_2")
        and getattr(pipeline, "tokenizer_2") is not None
    )


def extract_seq_hidden(model_output):
    # Prefer explicit hidden states for SDXL text_encoder_2, then fall back safely.
    if hasattr(model_output, "hidden_states") and model_output.hidden_states is not None:
        # SDXL pipelines typically use penultimate hidden state for conditioning.
        if len(model_output.hidden_states) >= 2:
    ...
    if isinstance(model_output, (tuple, list)):
        for item in model_output:
            if isinstance(item, torch.Tensor) and item.dim() == 3:
                return item
    raise ValueError("Cannot extract sequence hidden states from text encoder output.")


def get_text_hidden_states(pipeline, prompt, device):
    """
    Return tokenized inputs from primary tokenizer and hidden states used for cross-attention.
    - SD/SD2: [B, L, 768/1024]
    - SDXL:   [B, L, 2048] (concat of text_encoder and text_encoder_2 hidden states)
    """
    primary_inputs = get_token_id(prompt, pipeline.tokenizer, return_ids_only=False)

    if is_sdxl_pipeline(pipeline):
        secondary_inputs = get_token_id(prompt, pipeline.tokenizer_2, return_ids_only=False)
        primary_out = pipeline.text_encoder(
            primary_inputs.input_ids.to(device),
            output_hidden_states=True,
            return_dict=True,
        )
        secondary_out = pipeline.text_encoder_2(
            secondary_inputs.input_ids.to(device),
            output_hidden_states=True,
            return_dict=True,
        )
        primary_hidden = extract_seq_hidden(primary_out)
        secondary_hidden = extract_seq_hidden(secondary_out)
        hidden_states = torch.cat([primary_hidden, secondary_hidden], dim=-1)
    else:
        primary_out = pipeline.text_encoder(
            primary_inputs.input_ids.to(device),
            output_hidden_states=True,
        ...
        hidden_states = extract_seq_hidden(primary_out)

    return primary_inputs, hidden_states
...
def infer_text_embed_dim(pipeline):
    # Prefer UNet cross-attention dim because that is the matrix-multiply target.
...
def probe_attention_scores(args, pipeline, prompt, token_indices, device="cuda"):
    ...
    _, text_embeddings = get_text_hidden_states(pipeline, prompt, device)
    latents = torch.randn(
        1,
...
def edit_model(args, pipeline, target_concepts, anchor_concepts, retain_texts, baseline=None, chunk_size=128, emb_size=None, device="cuda"):
    ...
    if baseline in ['SPEED']:
        _, null_hidden_batched = get_text_hidden_states(pipeline, '', device)
        null_hidden = null_hidden_batched[0] # [seq_len, emb_size]
    ...
    for i in range(0, len(target_concepts)):
        target_inputs, target_hidden = get_text_hidden_states(pipeline, target_concepts[i], device)
        target_embs = target_hidden[0] # [seq_len, emb_size]
        anchor_inputs, anchor_hidden = get_text_hidden_states(pipeline, anchor_concepts[i], device)
        ...
        if args.mapping2context:
            ...
            for j in range(0, len(current_template)):
                ...
                anchor_inputs, anchor_hidden = get_text_hidden_states(pipeline, anchor_prompt, device)
                anchor_embs = anchor_hidden[0] # [seq_len, emb_size]
                anchor_embs = anchor_embs[[(anchor_inputs.attention_mask[0].sum().item() - 2)], :]  # last subject token [1, emb_size]
    ...
    for j in range(0, len(retain_texts), chunk_size):
        ret_inputs, ret_embs = get_text_hidden_states(pipeline, retain_texts[j:j + chunk_size], device) # [chunk_size, seq_len, emb_size]
        if retain_texts == ['']:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mapLastSubjectToken", action='store_true', default=False, help="Whether to map the anchor concept to the last subject token representation instead of the EoT token when mapping2context is True")
    parser.add_argument("--anchor_EoT", action='store_true', default=False, help="Whether to use the EoT token as the anchor representation when mapping2context is False")
    
    parser.add_argument("--erasetype", type=str, default="instance", choices=["instance", "style"], help="The type of erasure to perform, which determines the prompt templates used for mapping.")
    parser.add_argument("--mapAll", action='store_true', default=False, help="Whether to map the anchor concept using all templates (averaging their representations) when mapping2context is True")
    parser.add_argument("--mapMean", action='store_true', default=False, help="Whether to map the anchor concept using the mean representation of all templates when mapping2context is True (overrides mapAll if both are set)")
    ...
    try:
        pipeline = AutoPipelineForText2Image.from_pretrained(args.sd_ckpt).to(device)
    except Exception:
        # Fallback for older diffusers versions/checkpoints where auto pipeline may fail.
